...
- Component: vital-sign-blood-pressure-diastolic-as-mmhg-input
- PascalCase: VitalSignBloodPressureDiastolicAsMmhgInput
- Description: number input of one vital sign blood pressure diastolic with integers
- HTML tag: <input>
...
- Defaults: `min=0`, `max=200`, `step=1` for integer-only mmHg entry
- Supports `required` and `disabled` boolean states
- Spreads `restProps` onto the input for consumer customization
- Companion to VitalSignBloodPressureDiastolicAsMmhgView for the Input/View pattern
...
- `value`: number | undefined (default: `undefined`) -- current diastolic value in mmHg; bindable with two-way binding
- `min`: number (default: `0`) -- minimum allowed value
- `max`: number (default: `200`) -- maximum allowed value
...
- [ ] Renders <input> element with class="vital-sign-blood-pressure-diastolic-as-mmhg-input"
...
- CSS class: .vital-sign-blood-pressure-diastolic-as-mmhg-input in css-style-sheet-template.css
- HTML headless: lily-design-system-html-headless/components/vital-sign-blood-pressure-diastolic-as-mmhg-input.html
