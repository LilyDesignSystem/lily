...
Vue 3 + Nuxt 3 example application demonstrating all 316 components from the [Lily Design System](https://github.com/LilyDesignSystem/lily) headless component library, styled with [NHS UK design system](https://service-manual.nhs.uk/design-system) colors, typography, spacing, and focus states.

## Features

- 316 headless Vue components with `<script setup lang="ts">` syntax
...
- 1342 tests across 316 test files
...
Open [http://localhost:3160](http://localhost:3160).
...
```
lily-vue-nuxt-examples/
├── app.vue                     # Root layout
├── nuxt.config.ts              # Nuxt configuration (imports NHS CSS)
├── components/                 # 316 headless Vue components
│   ├── AccordionNav.vue
│   ├── Alert.vue
│   ├── Button.vue
│   └── ... (316 total)
...
├── tests/components/           # 316 component test files
├── package.json
├── tsconfig.json
├── vitest.config.ts
└── vitest-setup.ts
...
- **Component styles**: All 316 component CSS classes with NHS-appropriate styling
...
- [Vue Headless](../lily-vue-headless/) — 316 headless Vue components
